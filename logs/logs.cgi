...
privatekey = open("privatekey").readline()
...
def utf8ize(s):
    ...
    for nugget in s.split(' '):
        try:
            nuggets.append(unicode(nugget, 'utf-8'))
        except:
            try:
                nuggets.append(unicode(nugget, 'iso-8859-1'))
            except:
                nuggets.append(unicode(nugget))
...
def get_captcha(rresponse, rchallenge, ipaddress, pkey):
    tup = (pkey, ipaddress, rchallenge, rresponse)
    d = {}
    d['challenge'] = rchallenge
    d['response'] = rresponse
    d['privatekey'] = pkey
    d['remoteip'] = ipaddress
    url_values = urllib.urlencode(d)

    req = urllib2.Request('http://www.google.com/recaptcha/api/verify',
            url_values)
    response = urllib2.urlopen(req)
    success = response.read()
    success = success.splitlines()
    return success
...
def main():
    ...
    for x in form:
        print "item from form:", x
    if (os.environ['REQUEST_METHOD'] == "POST"):
        rresponse = form['recaptcha_response_field'].value
        rchallenge = form['recaptcha_challenge_field'].value
        ipaddress = os.environ['REMOTE_ADDR']
        pkey = privatekey
        response = get_captcha(rresponse, rchallenge, ipaddress, pkey)

        if (response[0] == "true"):
            if "reqdate" not in form:
                homepage()
            else:
                log(form['reqdate'].value)
